layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        elem: '#noticeList',
        url : '/ssm_stumanager/notice/list.html',
        request: {
        	pageName: 'curr' 
        	,limitName: 'nums'
        },
    ...
    function addNotice(edit){
        var index = layui.layer.open({
            title : "编辑公告",
            type : 2,
            content : "/ssm_stumanager/notice/addPage.html",
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的公告？', {icon: 3, title: '提示信息'}, function (index) {
                $.get("/ssm_stumanager/notice/deleteList.html",{
    ...
    table.on('tool(noticeList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除此公告？',{icon:3, title:'提示信息'},function(index){
                 $.get("/ssm_stumanager/notice/delete.html",{
                     id : data.id
                 },function(res){
                    tableIns.reload();
                    layer.close(index);
        	...
        	var index = layui.layer.open({
                title : "公告详情",
                type : 2,
                content : "/ssm_stumanager/notice/look.html",
                success : function(layero, index){
                    var body = layui.layer.getChildFrame('body', index);
